        ...
        if($filter_opt = "1"){
            ...
            && isset($_POST['clerk_ID'])){
                ...
                if($selectedTYPE=="s_type1"){
                    ...
                    CASE WHEN d.created_at IS NOT NULL THEN CONCAT('$start_date',' - ','$end_date')
                    ELSE 'NA'
                    END AS date_created
                    FROM
                        owners a
                        ...
                        if(empty($time_IN) && empty($time_OUT)){
                            $summary_sd_range .=" AND (d.created_at BETWEEN '$startDateTime' AND '$endDateTime')";
                        }else{
                            $summary_sd_range .=" AND (DATE(d.created_at) BETWEEN '$start_date' AND '$end_date')";
                        }
                        $summary_sd_range .=" AND d.void = '0'
                    WHERE
                    ";
                    if($selectedSD == "1"){
                        $summary_sd_range .="d.created_at IS NOT NULL";
                    }
                    if($selectedSD == "2"){
                        $summary_sd_range .="(d.created_at IS NOT NULL OR d.created_at IS NULL)";
                    }
                    if(!empty($dep_ID)){
                        $summary_sd_range .= " AND b.id = '$dep_ID'";
                    }
                    $summary_sd_range .="
                    GROUP BY
                        a.owner_name,
                    ...
                    if($result_summary_sd_range->num_rows > 0){
                        while($row_summary_sd_range = mysqli_fetch_assoc ($result_summary_sd_range)){
                            ...
                            <td><?php echo $row_summary_sd_range["date_created"];?></td>
                            </tr>
                            <?php
                        }
                    }
                ...
                if($selectedTYPE=="s_type2"){
                    ...
                    WHERE
                        ...
                        ";
                        if(!empty($time_IN) && !empty($time_OUT) ){
                            $breakdown_sd_range .= "AND d.created_at BETWEEN '$startDateTime' AND '$endDateTime'";
                        }else{
                            $breakdown_sd_range .= "AND (DATE(d.created_at) BETWEEN '$start_date' AND '$end_date')";
                        }
                        if(!empty($dep_ID)){
